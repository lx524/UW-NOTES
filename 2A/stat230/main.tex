\documentclass{article}

\usepackage{../../util}

\title{STAT 230 Probability}
\author{University of Waterloo - Spring 2024}

%\date{April 19, 2024}

\begin{document}
\maketitle

\tableofcontentsnewpage{}

\section{Introduction to Probability}
\input{01_intro_to_probability.tex}

\section{Mathematical Probability Models}
\input{02_mathematical_prob_models.tex}

\section{Probability and Counting Techniques}
\input{03_prob_and_counting_techniques.tex}

\section{Probability Rules and Conditional Probability}
\input{04_prob_rules_and_conditional_prob.tex}

\section{Discrete Random Variables}
\input{05_discrete_random_variable.tex}

\section{Computational Methods and the Statistical Software R}
\input{06_r.tex}

\section{Expected Value and Variance}
\input{07_expected_value_and_var.tex}

\section{Continous Random Variables}
\input{08_continuous_random_variables.tex}

\section{Multivariate Distributions}
\input{09_multivariate_distributions.tex}

\section{Central Limit Theorem and Moment Generating Functions}
\input{10_central_limit_and_moment_generating.tex}



\end{document}